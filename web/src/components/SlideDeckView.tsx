...
export const SlideDeckView: React.FC<SlideDeckViewProps> = ({
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-[#05070e] text-slate-100 overflow-hidden select-none">
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1 bg-slate-800/90 border border-slate-700 rounded-lg p-0.5 shadow-sm">
                ...
                <div className="flex items-center gap-1 px-1.5 text-xs font-bold font-mono text-slate-200">
                  ...
                  <span className="text-white text-xs font-bold min-w-[14px]">{totalSlides}</span>
                </div>
      ...
      {totalSlides > 0 && (
        <div className="min-h-[5.5rem] bg-slate-900/95 border-t border-slate-800 flex items-center shrink-0 shadow-inner overflow-hidden py-1">
          <div
            ref={filmstripRef}
            tabIndex={0}
            role="region"
            aria-label="投影片縮圖"
            className="flex items-center gap-3 overflow-x-auto overflow-y-hidden w-full h-full px-5 py-2 focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:outline-none"
          >
            {allSlides.map((slide, idx) => {
              ...
              return (
                <button
                  ...
                  className={`h-16 aspect-[4/3] rounded-lg overflow-hidden shrink-0 cursor-pointer relative transition-all duration-150 transform text-left p-0 focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:outline-none ${
                    isActive
                ...
                >
                  ...
                  <div className="absolute top-1 left-1 px-1.5 py-0.5 bg-slate-900/90 rounded text-xs font-bold text-slate-200 border border-slate-700/80">
                    {slide.slideNumber}
                  </div>

                  {slide.sectionTag && (
                    <div className="absolute bottom-1 right-1 px-1.5 py-0.5 bg-amber-500/90 rounded text-xs font-bold text-black truncate max-w-[4rem]">
                      {slide.sectionTag}
                    </div>
                  )}
                </button>
